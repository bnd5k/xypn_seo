  ...
  module XYPNScraper
    class FindOrCreateAdvisorWebsite
      ...
      def parse_advisors(html_string)
        advisor_url_array = ScrapeListOfAdvisors.new.parse(html_string)
      ...
      def parse_individual_advisor_pages(url_array)
        url_array.map do |xypn_url|
          ...
          advisor_info_hash =
            ScrapeAdvisorProfilePage.new.parse(response.body, xypn_url)
          advisor_info_hash
        end
      end
      ...
      def persist_advisors(advisors)
        advisors.each do |advisor|
          website =
            Website.find_or_initialize_by(xypn_profile: advisor[:xypn_profile])
